...
onValue(propertyRef, (snapshot) => {
  ...
  if (data) {
    ...
    Object.values(data).forEach((property) => {
      if (property.propertycategory.toLowerCase() !== "letting") {
        return;
      }
      ...
      getDownloadURL(imageRef).then((url) => {
        properties.innerHTML += `<div class="box-div">
              ...
              <a id="bedrent" data-id=${property.id} href=""> ${
  ...
  sortedLocations.forEach((location) => {
    ...
    locationOptions.innerHTML += `<option>${location}</option>`;
  });
});
