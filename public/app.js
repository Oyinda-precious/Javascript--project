...
const bedroomLink = document.getElementById("bedroom");
const propertyForSale = document.getElementById("propertyForSale");
const propertyForRent = document.getElementById("propertyForRent");
      ...
      if (data) {
        Object.values(data).forEach((value, index) => {
          if (index < 6) {
            ...
            bedLink.id = "bedroom";
            bedLink.href = "building.html";
            bedDiv.className = "bedroom";
            bedLink.textContent = value.propertyName;
            bedLink.setAttribute("building-data-id", value.id);
            bedLink.addEventListener("click", loadBuildingDetails);
            ...
            mSpan.textContent = value.propertycategory;
...
function loadBuildingDetails(event) {
  event.preventDefault();
  const buildingId = event.target.getAttribute("building-data-id");
  location.href = `building.html?buildingId=${buildingId}`;
}

function redirectForSale(event) {
  event.preventDefault();
  window.location.href = "forsale.html";
}

propertyForSale.addEventListener("click", redirectForSale);

function redirectForRent(event) {
  event.preventDefault();
  window.location.href = "forrent.html";
}
propertyForRent.addEventListener("click", redirectForRent);
